...
/**
 * An iterator which splits an {@link cc.mallet.types.InstanceList} into n-folds and iterates
 * over the folds for use in n-fold cross-validation. For each iteration,
 * list[0] contains a {@link cc.mallet.types.InstanceList} with n-1 folds typically used for
 * training and list[1] contains an {@link cc.mallet.types.InstanceList} with 1 fold typically
 * used for validation.
 * 
 * This class uses {@link cc.mallet.types.MultiInstanceList} to avoid creating a new
 * {@link cc.mallet.types.InstanceList} each iteration.
 * 
 * TODO - currently the distribution is completely random, an improvement would
 * be to provide a stratified random distribution.
 * 
 * @see cc.mallet.types.MultiInstanceList
...
implements java.util.Iterator<InstanceList[]>, Serializable {
     ...
     * 
     * @return A two element array of {@link cc.mallet.types.InstanceList}, where
