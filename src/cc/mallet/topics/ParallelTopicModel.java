 ...
 * User: Seonggyu Lee
 * Date: 14. 9. 30 오후 9:54
 * Created Date : $today.year.month.day
 * Last Modified : 14. 7. 3 오전 12:32
...
public class ParallelTopicModel implements Serializable {
	...
	public void optimizeAlpha(WorkerRunnable[] runnables) {
		...
		for (int thread = 0; thread < numThreads; thread++) {
			...
			for (int topic=0; topic < numTopics; topic++) {
				...
				if (! usingSymmetricAlpha) { //SymmetricAlpha true is better for MyExp
    ...
    public void printTopWordsForR (PrintStream out, int numWords, boolean usingNewLines) throws IOException {
        out.print(displayTopWordsForR(numWords, usingNewLines));
    }
	...
	public String displayTopWords (int numWords, boolean usingNewLines) throws IOException {
		...
		for (int topic = 0; topic < numTopics; topic++) {
			...
			if (usingNewLines) {
				...
				out.append (topic + "\t" + formatter.format(alpha[topic]) + "\t"+tokensPerTopic[topic] +"\t");
    ...
    public String displayTopWordsForR (int numWords, boolean usingNewLines) throws IOException {
...
//			builder.append("\t");
			...
			if (data.get(doc).instance.getName() != null) {
				builder.append(data.get(doc).instance.getTarget()); 
			}
			else {
     ...
     *  @param max		 Print no more than this many topics
     */
    public void printDocumentTopicsForMyExp (PrintWriter out,PrintWriter outForR,  int docNum, double threshold, int max)	{
        List<Integer> rLabelList = new ArrayList<Integer>();
        rLabelList.add(2);rLabelList.add(3);rLabelList.add(10);rLabelList.add(16); //target class 출력할 것들.
        ...
        for (int doc = 0; doc < data.size(); doc++) {
            ...
            StringBuilder builder = new StringBuilder();
            StringBuilder builderForR = new StringBuilder(); //forR
...
//			builder.append("\t");
            ...
            // forR
            if(previousTarget!=target.getIndex()) docNumPerEachClass=0;
            else docNumPerEachClass++;
            previousTarget=target.getIndex();
...
//            Arrays.sort(sortedTopics); (무조건 앞에서 부터 출력해야 하므로 주석담)
            ...
            //forR
            if(rLabelList.indexOf(target.getIndex()+1)!=-1 && docNumPerEachClass>3 && docNumPerEachClass<=(3+docNum)){
                for (int i = 0; i < max; i++) {
