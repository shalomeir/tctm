...
public class LDAHyper implements Serializable {
    ...
    public LDAHyper (int numberOfTopics, double alphaSum) {
...
//			Make sure it actually gets set
    ...
    protected void sampleBalancedTopicsForOneDoc (FeatureSequence tokenSequence,
        ...
        // for Balance Topic Weights
        for (int i = 0; i < numTopics ; i++) {
            localTopicWeights[i]=localTopicWeights[i]*balancePerTopic[i]; // updated for My Exp
        }
        ...
        for (int topic: localTopicCounts.keys()) {
            ...
            double nw = localTopicWeights[topic]; // for balanced topic model
            ...
            //			update the coefficients for the non-zero topics
//            cachedCoefficients[topic] =	(alpha[topic] + n) / (tokensPerTopic[topic] + betaSum);
            cachedCoefficients[topic] =	(alpha[topic] + nw) / (tokensPerTopic[topic] + betaSum); //For Balanced Topic Model
        ...
        for (int position = 0; position < docLength; position++) {
            ...
            cachedCoefficients[oldTopic] =
                    (alpha[oldTopic] + localTopicWeights[oldTopic]) /
                            (tokensPerTopic[oldTopic] + betaSum);
...
//			Make sure it actually gets set
            ...
            //			update the coefficients for the non-zero topics
            cachedCoefficients[newTopic] =
                    (alpha[newTopic] + localTopicWeights[newTopic]) /
                            (tokensPerTopic[newTopic] + betaSum);
    ...
    private double getKurtosFromMap(TIntIntHashMap currentTypeTopicCounts, int numTopics) {
        ...
        double kurtxMax = ctdCounts.length-1;

        return kurtx/kurtxMax;

    }

    public double getVarianceFromMap(TIntIntHashMap currentTypeTopicCounts, int numTopics) {
        double variance = 0;
        double sum = 0;
        ...
        for (int i = 0; i < ctdCounts.length; i++) {
            double x = ctdCounts[i]-1/ctdCounts.length;
            variance+= x*x; // math.pow is so slow.
        }

        double varianceMax = 1-1/ctdCounts.length;
    ...
    public void printTopWordsForR (PrintStream out, int numWords, boolean usingNewLines) throws IOException {
        //remove Stopwords
        BufferedReader stopReader = new BufferedReader(new FileReader("stoplists/forNewsgroups.txt"));
...
//        rLabelList.add(2);rLabelList.add(3);rLabelList.add(10);rLabelList.add(16); //target class 출력할 것들. for 20 Newsgroup
        ...
        // For My Exp Weighted Topic model
        double[] typeTopicWeight = new double[numTypes];
        for (int type=0; type < numTypes; type++) {
            TIntIntHashMap currentTypeTopicCounts = typeTopicCounts[type];  // this is important value similar with beta
...
//			builder.append("\t");
            ...
            if (data.get(doc).instance.getTarget() != null) {
                target = (FeatureVector) data.get(doc).instance.getTarget(); //TARGET 을 넘버로 출력
                builder.append(target.getIndices()[0]+1);
            }
            else {
                builder.append("0"); //모르면 0
